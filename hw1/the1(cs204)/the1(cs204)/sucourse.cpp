                    ...
                    if(alan <= count && alan !=0){  // burada hata oluşuyor.
                        for(int m =0 ; m < alan; m++){
                            ...
                            do{
                                ...
                                getline(cin, line);
                                string roww = line.substr(0,1);
                                string columnn = line.substr(2,1);
                                int newrow =stoi(roww);
                                int newcolumn =stoi(columnn);
                                
                               
                                
                                //cin >> row >> column;
                                
                                if(!(newrow >= 0 && newrow < colony.size()) || !(newcolumn >= 0 && newcolumn < colony[0].size()) ){
                                    ...
                                    if(!(colony[newrow][newcolumn] == '-')){
                                        ...
                                        colony[newrow][newcolumn] = girdi; // burada hata oluşuyor.
                                        //cout << "The building is added to the colony." << endl;
                                        char onceki = colony[newrow][newcolumn];
                                        //colony[row][column] = '-';
...
void removebuilding(vector<vector<char>>& colony,vector<vector<string>>& stock,vector<vector<string>>& consumption){
    ...
    if(bul == true){
        ...
        //!(newrow >= 0 && newrow < colony.size()) || !(newcolumn >= 0 && newcolumn < colony[0].size()) || colony[newrow][newcolumn] == '-'
        }while(dong == false);
        
        cout << "The building is removed, and the corresponding resources are added back to the stock." << endl;
        
        char onceki = colony[newrow][newcolumn];
        colony[newrow][newcolumn] = '-';
        string onceki_str = string(1, onceki);
        //cout <<  "??????????" << endl;
