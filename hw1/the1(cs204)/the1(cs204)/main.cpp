...
void addbuilding(vector<vector<char>>& colony,vector<vector<char>> copycolony){
    ...
    if(bul == true){
        ...
        do{
            ...
            if((girdi >= 'A' && girdi <= 'Z') && varmi == true){
                ...
                if(alan <= count){  // burada hata oluşuyor.
...
void removebuilding(vector<vector<char>>& colony,vector<vector<string>>& stock,vector<vector<string>>& consumption){
    ...
    if(bul == true){
        ...
        colony[row][column] = '-';
        string stronceki = to_string(onceki);
        
        
        for(int j=0; j < consumption.size(); j++){
            for(int k=0; k < consumption[0].size(); k++){
                
                if(consumption[j][0] == stronceki){
...
void resourceupdate(vector<vector<string> > &stock, vector<vector<string>> consumption, vector<vector<char>> colony) {
    ...
    vector<vector<string>>cikarilcak;
    vector<int> toplam;
    vector<string> temp;
...
int main(){
    ...
    cout << "Resources consumption per building type loaded from " << stockdatabase << endl;
